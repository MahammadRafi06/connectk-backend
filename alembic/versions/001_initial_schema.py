...
# Define enums using postgresql.ENUM with create_type=False.
# The types are created explicitly via op.execute() below so that
# we can use IF-NOT-EXISTS-style error handling for race-safety.
_cloud_provider = postgresql.ENUM("GKE", "AKS", "EKS", name="cloud_provider", create_type=False)
_cluster_status = postgresql.ENUM("active", "unreachable", "pending", name="cluster_status", create_type=False)
_gitops_tool = postgresql.ENUM("argocd", "fluxcd", name="gitops_tool", create_type=False)
_deployment_backend = postgresql.ENUM("sglang", "vllm", "trtllm", name="deployment_backend", create_type=False)
_deployment_type_enum = postgresql.ENUM("aggregated", "aggregated_route", "disaggregated_route", name="deployment_type_enum", create_type=False)
_deployment_status = postgresql.ENUM(
...
_model_source_type = postgresql.ENUM("huggingface", "s3", "gcs", "azure_blob", "custom", name="model_source_type", create_type=False)
_access_level = postgresql.ENUM("list", "view", "deploy", "admin", name="access_level", create_type=False)
...
def upgrade() -> None:
    # Create enum types explicitly with race-safe DO blocks
    ...
    op.create_table(
        ...
        sa.Column("provider", _cloud_provider, nullable=False),
        ...
        sa.Column("status", _cluster_status, nullable=False, server_default="pending"),
        sa.Column("cache_ttl_seconds", sa.Integer, server_default="300"),
        sa.Column("gitops_tool", _gitops_tool, nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("source_type", _model_source_type, nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("backend", _deployment_backend, nullable=False),
        sa.Column("deployment_type", _deployment_type_enum, nullable=False),
        ...
        sa.Column("status", _deployment_status, nullable=False, server_default="creating"),
    ...
    op.create_table(
        ...
        sa.Column("access_level", _access_level, nullable=False),
